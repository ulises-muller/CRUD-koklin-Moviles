        ...
        setContent {
            MyApplicationTheme {
                ...
                NavHost(navController, startDestination = "Sistema") {
                    composable("Sistema") { Sistema(navController, ::mostrarDialogoError) }
                }
            }
        }
    }
    /*funcion cartelito error*/

    fun mostrarDialogoError(){
        val builder = AlertDialog.Builder(this)
        builder.setTitle("ERROR")
            .setMessage("El dni no es el correcto")
...
typealias ShowErrorDialog = () -> Unit
@Composable
private fun Sistema(navController: NavHostController, showErrorDialog: ShowErrorDialog) {

    val texto by remember { mutableStateOf("Miembros") }
    var textoInput by remember { mutableStateOf("") }
    val tasks = remember { mutableStateListOf<Task>() }
    var dniInput by remember { mutableStateOf("") }
    var datosCompletar  by remember { mutableStateOf<Task?>(null) }
    fun reload(){
    ...
    Column(
        ...
        Column(
            ...
            Button(onClick = {
                datosCompletar?.let { dato ->
                    if (dato.dni == dniInput){
                        modificarUsuario(textoInput, dato.id, dniInput, dato.nroMembresia)
                        reload()
                    } else {
                        showErrorDialog()
                    }
        ...
        /* Datos y botones de los usuarios */
            ...
            LazyColumn(
                ...
                itemsIndexed(tasks) { index, task ->
                    ...
                    Row{
                        ...
                        IconButton(onClick = {
                            datosCompletar = task
                            textoInput = task.task
                            dniInput = task.dni
                        }) {
                            Icon(Icons.Default.KeyboardArrowUp, contentDescription = "Autocompletar datos")
                        }
                    }
                }
            }
